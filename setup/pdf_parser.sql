...
CREATE OR REPLACE TABLE WHOOP.DOCUMENTS.docs_chunks_table AS
    SELECT
        relative_path,
        build_scoped_file_url(@WHOOP.DOCUMENTS.PDFS, relative_path) AS file_url,
        -- preserve file title information by concatenating relative_path with the chunk
        CONCAT(relative_path, ': ', func.chunk) AS chunk,
        'English' AS language
    FROM
        directory(@WHOOP.DOCUMENTS.PDFS),
        TABLE(WHOOP.public.pdf_text_chunker(build_scoped_file_url(@WHOOP.DOCUMENTS.PDFS, relative_path))) AS func;
